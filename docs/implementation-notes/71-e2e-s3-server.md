...
`TestBootstrap` passed four times with the test's assertions unchanged: twice locally (`KIND_EXPERIMENTAL_PROVIDER=podman`, 422 s and 414 s) and twice in CI (`kind bootstrap`, 7m49s and 8m11s). In every run, `shop-staging`'s scheduled Backup reached `completed` against versitygw.

In the second CI run, the final-backup hook's own Backup (`shop-staging-final-20260924134312`) also reached `completed` before the Cluster was deleted, which is the pass `testDeleteEnvironment` exists to prove. In the other three runs, ArgoCD deleted `shop-staging` before the hook's Backup appeared: the argoproj/argo-cd#29100 race, which the test tolerates by design (#39's notes). Nothing in those runs points at the server.
