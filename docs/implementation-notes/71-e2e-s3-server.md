...
`TestBootstrap` passed three times with the test's assertions unchanged: twice locally (`KIND_EXPERIMENTAL_PROVIDER=podman`, 422 s and 414 s) and once in CI (`kind bootstrap`, 7m49s). In each run, `shop-staging`'s scheduled Backup reached `completed` against versitygw. In each run, ArgoCD also deleted `shop-staging` while the final-backup hook's Job was active or just created, before its own Backup appeared. That is the argoproj/argo-cd#29100 race the test tolerates by design (#39's notes). So these runs prove the hook ran, and that a Barman Backup completes against versitygw, but not the hook's own Backup reaching `completed`. That Backup is the same kind of object as the scheduled one (`method: plugin`, the same Barman Cloud plugin and ObjectStore), so versitygw has nothing different to serve for it.
