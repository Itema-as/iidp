...
2. **Bump the bootstrap.** Set `targetRevision` in the Platform repository's `bootstrap/platform-components.yaml` to that tag. ArgoCD installs `deploy-gate` in `argocd`, external-dns creates `deploy.<baseDomain>`, and the wildcard covers it. Check that `curl https://deploy.<baseDomain>/healthz` answers `ok`, and read the pod's first log line for the audience, issuer and org id.
3. **Bind `hello`.** `hello` must be in `Itema-as`; transfer it first if it is not. Then run `iidp app bind hello --repo Itema-as/hello`, with the CLI from step 1 or any release since #58.
4. **Change `hello`'s workflow.** Regenerating it from the step 1 release is simplest. By hand, in `.github/workflows/deploy.yaml`:
