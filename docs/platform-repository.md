...
The gate also pins `Itema-as`'s id (`1230559`) in its own configuration (`deployGate.githubOrgId`, `bootstrap/values.yaml`) and requires both the token's `repository_owner_id` and the binding's `repositoryOwnerId` to equal it. That defends against a hand edit that binds a repository outside the org, although anyone able to make that edit can already write to the Platform repository.
...
- **`--login`** sets `login.enabled: true` in every Environment (prod and, with `--staging`, staging too: one oauth2-proxy cookie covers both, `docs/implementation-notes/18-itema-login.md`), and `platform.loginCookieDomain` to the login cookie's domain: `platform.yaml`'s `cloudflareZone`, or `baseDomain` without one, the same rule the bootstrap's oauth2-proxy follows. The chart checks custom domains against it. Every `--domain` must be inside it: `--login --domain x.itma.no` works, `--login --domain shop.example.com` is refused, naming the domain, before anything is created (on `--path create` and `--path adopt`, before the Application repository or the pull request). A domain outside the zone never gets the cookie, so its users would go back to sign in forever. Login on such a domain would need the sign-in flow on each host and an Entra redirect URI per domain, which the Platform does not do.
...
## How a deploy reaches the Platform repository: the Deploy gate

An Application repository's CI never writes the Platform repository itself. Its deploy workflow (`.github/workflows/deploy.yaml`, `internal/templates`) runs `iidp ci set-image <app> <prod|staging|auto> <tag>` after it pushes an image to GHCR, and that command asks the **Deploy gate** to make the change ([ADR-0005](adr/0005-private-application-repositories-on-github-free.md), [`docs/implementation-notes/60-deploy-gate.md`](implementation-notes/60-deploy-gate.md)). The gate is a small service on the Platform (`cmd/iidp-deploy-gate`, installed by the bootstrap, `bootstrap/README.md`) at `https://deploy.<baseDomain>`, and it is the only holder of the `iidp-deploy` GitHub App's key outside ArgoCD.
...
**The gate's side.** For each call it checks, in order, and refuses at the first failure:

1. The token: RS256, signed by a key the issuer (`https://token.actions.githubusercontent.com`) publishes in its key set (fetched and cached), issued by that issuer, for the gate's own URL as audience, and not expired. Otherwise HTTP 401.
2. The caller: the token's `repository_owner_id` is `Itema-as`'s id, pinned in the gate's configuration (HTTP 403).
3. The ref: `refs/heads/main` deploys, `refs/tags/v*` promotes; any other ref is refused (HTTP 403).
4. On a fresh clone of `main`: the Application exists (HTTP 404), and it is bound (`applications/<name>/repository.yaml`, above) with both ids, to a repository in `Itema-as` whose id is the token's `repository_id` (HTTP 403, an unbound Application naming `iidp app bind`).
...
An Application's migration command belongs to its code: the two Environments run different code between a push to `main` and the next `v*` tag, and a command set before the image with its tooling is deployed fails that Environment's sync. So the command lives in the Application repository, in `iidp.yaml` at its root, and arrives with every Deploy and Promote ([`docs/implementation-notes/66-migration-command-in-repo.md`](implementation-notes/66-migration-command-in-repo.md)):

```yaml
# (a comment saying when and where it runs)
...
|---|---|---|
| no `iidp.yaml` | no `migrationCommand` | leaves it as it is |
| `iidp.yaml` without `migrationCommand`, or with `""` or null | `"migrationCommand": ""` | clears it: no migration |
| `iidp.yaml` with `migrationCommand: <command>` | `"migrationCommand": "<command>"` | sets it |

"No file" (keep) and "no line" (clear) differ on purpose. An Application made before the file existed, or one that deletes it, keeps working unchanged, while deleting the line is how a developer says "no migration". The gate applies the command's own rules on top of the checks above, and only to the Environment the caller may deploy:
