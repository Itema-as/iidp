...
2. Requests a GitHub Actions OIDC token whose audience is exactly that URL (`ACTIONS_ID_TOKEN_REQUEST_URL` with `audience=<url>`, authenticated with `ACTIONS_ID_TOKEN_REQUEST_TOKEN`).
3. Calls `POST <gate>/v1/deploy` with `Authorization: Bearer <token>` and `{"application": "<app>", "environment": "<prod|staging|auto>", "tag": "<tag>"}`. On a refusal it prints the gate's message, which says what was refused and why, and fails. A gate that cannot be reached, or answers 502, 503 or 504, is tried twice more; repeating a deploy is harmless.
...
Only then does it edit `image.tag` in place in the Environment's `values.yaml` (`render.SetImageTag`, every other key and comment untouched) and commit `Deploy <app> <environment> <tag>`, with a body naming the calling repository, its commit, the ref and the workflow. The author is the token's `actor`, `<actor_id>+<actor>@users.noreply.github.com`; the committer is the App's bot, `iidp-deploy[bot] <<bot id>+iidp-deploy[bot]@users.noreply.github.com>`. It pushes with an installation token minted for the call, and when `main` moved it clones afresh, checks everything in step 4 and 5 again and retries once, as every CLI command does. An Environment that already runs the tag gets no commit. The tag itself must be a valid image tag; checking that the image exists in GHCR is #61.
