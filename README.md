...
That is the Phase 1 target described in [`docs/design.md`](docs/design.md); the CLI is being built towards it ticket by ticket. Today it provides `iidp version` and `iidp app create`, with a Create path that generates the Application repository from a built-in framework template. Adopt (an existing Application repository, changed through a pull request) is not built yet. The vocabulary used throughout this repository is defined in [`CONTEXT.md`](CONTEXT.md).
...
Today that is all it does: Adopt, the remaining Capabilities and the interactive wizard come with later tickets. The Environment deploys once a deploy workflow writes the first image tag.
