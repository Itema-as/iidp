...
- Any other host (`shop.example.com`, `shop.itma.no`, or a deeper `test.shop.app.itma.no`, since a wildcard covers one label) goes on a second Ingress named `<name>-http01`, annotated `cert-manager.io/cluster-issuer: <platform.httpIssuer>`, with one TLS entry per host and its own secret `<name>-<host with dots as dashes>-tls`. cert-manager's ingress-shim issues one Certificate per entry, solved over HTTP-01. The hosts sit on a second object because ingress-shim acts on every host of the Ingress it finds the annotation on; kept apart, it never tries to issue for the wildcard hosts.
