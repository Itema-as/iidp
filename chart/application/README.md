...
Only the deploy workflow's write-back sets the tag, and it refuses an empty one. Emptying the tag of an Environment that already has one, by hand, would make the chart render nothing and ArgoCD's automated prune delete the running Environment, its database included, the same as removing any other object from the values file by hand would.
