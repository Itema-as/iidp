...
Only the deploy workflow's write-back sets the tag, and it refuses an empty one. If someone empties the tag of an Environment that already has one by hand, the chart renders nothing and ArgoCD's automated prune removes the running Application. The database survives: see "Keeping the database" below.
