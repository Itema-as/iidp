...
With `postgres.enabled`, the chart also renders a `ServiceAccount`, a `Role`, a `RoleBinding` and a Job, all named `<name>-final-backup`, annotated `argocd.argoproj.io/hook: PreDelete`. ArgoCD creates a `PreDelete` hook only when the Environment's own ArgoCD Application (the one `iidp app delete` removes from the Platform repository) is itself deleted, never during an ordinary sync; it waits for every `PreDelete` hook to reach Healthy before it deletes anything else, and a failing hook blocks the deletion (a `DeletionError` condition) until it is fixed in git or the hook resource is removed by hand. The `ServiceAccount`, `Role` and `RoleBinding` carry `argocd.argoproj.io/sync-wave: "-1"`, one wave before the Job's default wave, so the RBAC exists before the Job's Pod starts; `argocd.argoproj.io/hook-delete-policy: BeforeHookCreation,HookSucceeded` cleans a successful attempt's hook resources up and replaces a previous attempt's before a retry creates new ones.
