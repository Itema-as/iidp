...
| `kind` | `web-service` | What the Application is: `web-service` (a container listening on `port`) or `static-site` (an nginx image built by CI, listening on 8080 with `runAsNonRoot`, on 80 without). Anything else fails. |
...
| `port` | `3000` | The port a Web service listens on. Ignored by a Static site, whose port follows from `runAsNonRoot` (see "Port and probes" below). |
| `probe.path` | `/` | The path the readiness and liveness probes request. |
| `runAsNonRoot` | `false` | Whether the image runs as a non-root user with a numeric UID, as images built from iidp's Next.js and Vite React templates do. `true` makes every container pass Pod Security's `restricted` level and moves a Static site to 8080; see "Security context" below. `iidp app create` writes `true` when it generated the Dockerfile from one of those templates, and nothing otherwise. Anything but a boolean fails. |
| `env` | `{}` | Plain environment variables, name to value. Not for secrets, and it must not set `PORT` (a Web service gets it from `port`, and a Static site's port is fixed) nor `DATABASE_URL` when Postgres is enabled. |
...
| `tasks` | `[]` | Scheduled tasks, each `name`, `schedule` (five cron fields, Europe/Oslo time) and `command`, one CronJob each. See "Scheduled tasks" below. Needs `kind: web-service`; on a Static site it fails rendering, and so does a name that is not a DNS label, is used twice, or makes the CronJob's name longer than 52 characters, and a task without a schedule or a command. On the Platform the Deploy gate writes it with each deploy, from the Application repository's `iidp.yaml` ([`docs/platform-repository.md`](../../docs/platform-repository.md#scheduled-tasks-travel-with-the-deploy)). |
| `runTasks` | `true` | `false` renders no CronJob, whatever `tasks` holds: Preview Environments, which share staging's values, run no tasks. The CronJob name's length is then not checked either. |
...
**Port and probes.** A Web service container is told its port through the `PORT` environment variable, taken from `port`, and the Service listens on the same number. A Static site is an nginx image, not configured through the environment: `port` is ignored and no `PORT` is injected. Its port follows from who nginx runs as. With `runAsNonRoot`, it is the unprivileged nginx of iidp's Vite React template (`nginxinc/nginx-unprivileged`), which listens on **8080**, since a non-root process cannot bind a port below 1024. Without it, it is an nginx running as root, which is what every Static site built before #90 and any Adopted one runs (Itema-as/iprofil's own Dockerfile, for example), and it listens on **80**, as before. Nothing an existing Static site carries changes, so it keeps working on 80 without an edit. `port` was not used for this because every Static site the CLI wrote before #90 carries `port: 3000`, which it never listened on. Readiness and liveness probes are HTTP GETs to `probe.path` on the container's port for both Kinds; most Applications need no probe configuration because `/` answers.

**Security context.** Every container the chart renders (the Deployment's, the migration Job's, the final Backup hook's, each Scheduled task CronJob's, and any workload added later) takes its `securityContext` from one helper, `application.securityContext` in `_helpers.tpl`. It always sets the `RuntimeDefault` seccomp profile and `allowPrivilegeEscalation: false`, which no image that runs as root needs either. With `runAsNonRoot` (the Application's own image), and always for the final Backup hook's `bitnami/kubectl` (uid 1001), it also sets `runAsNonRoot: true` and drops every capability, which is everything Pod Security's `restricted` level asks of a container. Without it, the chart does not force `runAsNonRoot` and keeps the container runtime's default capabilities. nginx running as root needs `CHOWN`, `SETUID` and `SETGID` to start and `NET_BIND_SERVICE` for port 80, and an Adopted image may need others, so dropping `ALL` and adding back only `NET_BIND_SERVICE` would stop such images starting (tried with `nginx:1.30-alpine`: `chown("/var/cache/nginx/client_temp", 101) failed (1: Operation not permitted)`). Such a container passes `baseline`, which Application namespaces enforce, and is reported against `restricted`, which they warn and audit ([`bootstrap/README.md`](../../bootstrap/README.md#guardrails)). The kubelet can only check `runAsNonRoot` against a numeric `USER`: an image with `USER nextjs` and `runAsNonRoot: true` fails to start ("cannot verify user is non-root"), which is why the Next.js template now says `USER 1001:1001`. An Application generated from the Next.js template before #90 (hello) has the named user, and must change that line before its values say `runAsNonRoot: true`.

**Declared domains.** Every Environment with an image renders a ConfigMap named `iidp-domains` whose keys are its custom domains, empty when it has none, in sync wave -1 so it is there before the Ingresses. The bootstrap's Ingress-host admission policy reads it from the Ingress's own namespace and allows a host outside the Platform's base domain only when it is a key there. The name is fixed by the policy, so there is one Environment per namespace, as the CLI writes them.
...
**Scheduled tasks.** Each entry of `tasks` is a CronJob named `<name>-<task>` (`shop-nightly-cleanup`, `shop-staging-nightly-cleanup`), applied with the Application's other objects in wave 0. Its runs are Jobs from the Application image running `sh -c "<command>"` with the Application's `env`, its `secrets` as `envFrom`, and `DATABASE_URL` with Postgres on, but no `PORT`, since a task listens on nothing. Each run gets the `small` size's 250m CPU and 256Mi memory as requests and limits, whatever the Application's size. The settings are chart constants, checked against the Kubernetes documentation for the node's 1.36 (`docs/implementation-notes/91-scheduled-tasks.md`):

| Field | Value | Why |
|---|---|---|
| `timeZone` | `Europe/Oslo` | Tasks run on Norwegian time, not the node's. A time from 02:00 to 02:59 is skipped on the day clocks go forward and runs twice on the day they go back. |
| `concurrencyPolicy` | `Forbid` | A run still going when the next is due makes the next one skip; runs never overlap. |
| `startingDeadlineSeconds` | `300` | A run that cannot start within five minutes of its time is skipped, not made up late. It also keeps the controller from counting more than a few missed times after an outage (it stops at 100). |
| `successfulJobsHistoryLimit` / `failedJobsHistoryLimit` | `1` / `3` | The last success and the last three failures are kept with their Pods and logs; older runs are deleted. |
| `backoffLimit` | `0` | A failed run is not retried; the next time on the schedule runs the task again. |
| `activeDeadlineSeconds` | `3600` | A run is stopped and fails after an hour. |

The CronJob, its Jobs and their Pods carry `iidp.itema.no/application`, `iidp.itema.no/environment` and `iidp.itema.no/task` (the task's name) and `app.kubernetes.io/component: scheduled-task`, so Alloy attributes a run's logs; the Pods do not carry the selector labels. ArgoCD's health check for a CronJob reads Degraded when its last run failed after an earlier success, so a failing task shows on the Environment's ArgoCD Application. With `runTasks: false` no CronJob renders.

## An Environment without an image

`iidp app create` writes every Environment with `image.tag: ""`: no image exists until the first deploy through the Deploy gate (`iidp ci set-image`), which for prod is the next push to `main`, or, with a staging Environment, the first `v*` tag, when the workflow promotes staging's image. An Adopt Environment waits for its pull request to be merged, and `iidp app add-capability --staging` starts staging the same way. The empty tag is the signal: while it is empty the chart renders **no objects at all**. Not the Deployment, Service or Ingresses, and not the Postgres `Cluster`, `ObjectStore`, `ScheduledBackup`, migration Job, final-backup hook or Scheduled tasks' CronJobs either. Every template checks `application.released` (`_helpers.tpl`) after `application.validate`, so a broken values file (an unknown size, a missing `image.repository`, a missing bucket with Postgres on, ...) is still refused with the same message whether or not the Environment has an image yet.
...
helm template shop chart/application --values chart/application/testdata/tasks-staging-postgres.yaml
